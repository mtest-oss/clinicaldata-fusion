...
class LSTM(nn.Module):
    def __init__(self, args):
        ...
        if args.use_unstructure:
            # nn.Embedding (args.unstructure_size, args.embed_size )
            nn.Embedding (1000, args.embed_size ) #Manasa 1000
            print("args:content: nn.Embedding firstlayer = ", 1000, "args.emeb.size", args.embed_size)
    ...
    def forward(self, x, t, dd=None, content=None):
        ...
        if dd is None:
            #print("In forward:: content is not none!!")
        ...
        if content is not None:
            ...
            content, _ = self.lstm1(content)
            content = self.vocab_mapping(content) #Manasa this is commented to verify
            content = torch.transpose(content, 1, 2).contiguous()
            content = self.pooling(content)
            content = content.view((content.size(0), -1))
            out = self.cat_output(torch.cat((output, content, d), 1)) #Manasa this is commented to verify
        #else:
        #  print("No content in output")
