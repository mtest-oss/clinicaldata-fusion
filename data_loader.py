...
class DataBowl(Dataset):
    def __init__(self, args, files, phase='train'):
        ...
        self.files = files
        self.n_dd = 6
        ## Debug: modified 'w' to 'r'
        #args.unstructure_size = len(vector_dict)
        #print("args unsize", args.unstructure_size)


        #with open('/content/drive/MyDrive/clinical-fusion-master/data/processed/files/feature_mm_dict.json', 'rb') as f:
        #  data = f.read()
        #  data_str = data.decode("utf-8", errors='ignore')
        #  json.load(data_str)

        #print("args.files_dir", args.files_dir)
        
        #json.load(open('/content/drive/MyDrive/clinical-fusion-master/data/processed/files/feature_mm_dict.json', 'w'))
        #print("Filename: ", os.path.join(args.files_dir, 'feature_mm_dict.json'))
        self.feature_mm_dict = json.load(open(os.path.join(args.files_dir, 'feature_mm_dict.json'), 'r'))
        #print("init next")
        self.feature_value_dict = json.load(open(os.path.join(args.files_dir, 'feature_value_dict_{:d}.json'.format(args.split_num)), 'r'))
    ...
    def get_mm_item(self, idx):
        ...
        if len(input_list) < self.args.n_visit:
            for _ in range(self.args.n_visit - len(input_list)):
                # pad empty visit
                #print("len of input_list", len(input_list), "self.args.n_visit", self.args.n_visit)
        ...
        #args.unstructure_size = 1000  ##Added Manasa
