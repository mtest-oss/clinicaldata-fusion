...
def get_ids(split_json):
    ...
    train_ids = [adm_id[-12:-6] for adm_id in train_ids]
    val_ids = [adm_id[-12:-6] for adm_id in val_ids]
    test_ids = [adm_id[-12:-6] for adm_id in test_ids]
...
def get_ids2(split_json, seed):
    ...
    val_ids = np.hstack([adm_ids[t] for t in splits[7:8]])
    test_ids = np.hstack([adm_ids[t] for t in splits[8:]])
    train_ids = [adm_id[-12:-2] for adm_id in train_ids]
    val_ids = [adm_id[-12:-2] for adm_id in val_ids]
    test_ids = [adm_id[-12:-2] for adm_id in test_ids]
    return train_ids, val_ids, test_ids
